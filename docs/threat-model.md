...
| Signature replay or signer-pair substitution | EIP-712 domain separation plus deployment ID, frozen bounty fields, reward, verifier pool, and exact paid signer bitmap are signed. | Clients must sign the exact typed data and reject `personal_sign` substitutions. |
...
| A later ERC-20 deposit subsidizes old insolvency | Creation requires pre-deposit solvency, exact balance increase, and post-deposit backing before recording a bounty. | Rebases, clawbacks, or later token upgrades can still freeze an existing deployment; new funding fails closed. |
| Deflationary or deceptive withdrawal | Withdrawal requires pre/post solvency and verifies exact escrow decrease and destination increase; failure rolls back credit. | Blacklisting, pauses, callbacks, or dishonest `balanceOf` remain unsupported. |
